...
export class AuthHelperSalesforce extends AuthHelper implements TnsOAuth.ITnsAuthHelper {
  ...
  public logout(successPage?: string, cookieDomains?: Array<string>): Promise<void> {
    if (!cookieDomains) {
      cookieDomains = [".salesforce.com", ".force.com"];
    }
    return this._logout(successPage, cookieDomains);
  }

  // revokes token using Salesforce APIs (doesn't remove webview cookies)
